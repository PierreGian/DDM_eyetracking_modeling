...
path_output_VE = [path_output 'VE/'];
path_DDM_output = [master_path 'outputs/DDM_VE/'];
path_plots = [master_path,'plots/DDM_VE/'];
path_RT_plots = [master_path,'plots/DDM_VE/RT/'];
...
ToLoad = [path_output_VE 'weightsAmp_acc.mat'];
load(ToLoad)
w_acc = ws;
w_acc = abs(w_acc);
%Load weights RT
% ToLoad = [path_output_VE 'weights_RT.mat'];
% load(ToLoad)
% w_RT = ws;
...
Txt_file = [path_output,'AllDDMparams_wAcc_',analysis_type,'.csv'];
...
BIC = struct();
BIC.DDM = struct();
BIC.DDM.Amp4 = zeros(1,length(X));
BIC.DDM.Amp8 = zeros(1,length(X));
...
%for n=1:5
    ...
    % the 8th column is the mean VE pooled from left & right cases
    ...
    for k = 1:length(fields)
        ...
        thetaGuess = [.1 20 1 400 0.5];   % starting point bias (3rd param)
        ...
        [thetaFit,errFits(k),xflg,oput] = fminsearch('fitDiffMod_wVE', thetaGuess, opts, data.(fields{k}), fit_flags);
    ...
    for jj=[1:length(fields)]
        ...
        All_veW(jj,n) = thetafitvalues{jj}(5);
    ...
    DDM_AllRes.DDMVE.(cur_name) = struct();
    DDM_AllRes.DDMVE.(cur_name).psych.thresh = threshold_DDM;
    DDM_AllRes.DDMVE.(cur_name).psych.bias = bias_DDM;
    DDM_AllRes.DDMVE.(cur_name).psych.x_fit = x_fit;
    DDM_AllRes.DDMVE.(cur_name).psych.ppred = ppred;
    DDM_AllRes.DDMVE.(cur_name).psych.pmf = pmf;
    ...
    DDM_AllRes.DDMVE.(cur_name).RT.cmf = cmf;
    DDM_AllRes.DDMVE.(cur_name).RT.rt_data = rt_data;
    DDM_AllRes.DDMVE.(cur_name).RT.x_fit = x_fit;
    DDM_AllRes.DDMVE.(cur_name).RT.cmf_pred = cmf_pred;
    DDM_AllRes.DDMVE.(cur_name).RT.t1pred = t1pred;
    DDM_AllRes.DDMVE.(cur_name).RT.t2pred = t2pred;
    ...
    close(p)
    writematrix ([bias_DDM threshold_DDM], [path_DDM_output subject_list{n} '_bias_threshold_DDM_VE.csv']);
    All_bias(:,n) = bias_DDM';
...
bic_struc = fullfile(path_DDM_output, ['BIC.mat']);
save(bic_struc,'BIC')
rtbic_struc = fullfile(path_DDM_output, ['RTBIC.mat']);
