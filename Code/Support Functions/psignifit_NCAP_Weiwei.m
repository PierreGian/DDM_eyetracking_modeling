function result=psignifit_NCAP_Weiwei(data,options)
...
if ~isfield(options,'expType'),              options.expType        = 'YesNo';        end %want to make sure this is correct
if ~isfield(options,'estimateType'),         options.estimateType   = 'MAP';          end %want to make sure this is correct
...
if ~isfield(options,'threshPC'),             options.threshPC       = .5;             end %want to make sure this is correct
...
if ~isfield(options,'fixedPars'),            options.fixedPars      = [nan nan 0 0 nan]';              end%nan(5,1);       end
